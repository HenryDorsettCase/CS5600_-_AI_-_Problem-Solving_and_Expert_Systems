...
class Environment {
	...
	public final GridState getState() {
		char[][][] wumpusWorldCopy = new char[worldSize][worldSize][4];
				
		// store world definition
		for (int i = 0; i < worldSize; i++) {
			for (int j = 0; j < worldSize; j++) {
				for (int k = 0; k < 4; k++) {
					wumpusWorldCopy[i][j][k] = wumpusWorld[i][j][k];	
				}
			}
		}
		...
		final GridState state = new GridState(worldSize, wumpusWorld, haveGold);
		return state;
	}

	public GridPercepts getGridPercepts() {
		char[][][] perceptMapCopy = new char[worldSize][worldSize][4];

		for (int i = 0; i < worldSize; i++) {
			for (int j = 0; j < worldSize; j++) {
				for (int k = 0; k < 4; k++) {
					perceptMapCopy[i][j][k] = percepts[i][j][k];	
				}
			}
		}
		
		return new GridPercepts(perceptMapCopy, worldSize);
	}
}