...
class WorldApplication {
	...
	{		
		...
		boolean unlimitedArrows = false;

		String outFilename = "wumpus_out.txt";
		try 
		{
			fout = new BufferedWriter(new FileWriter(outFilename, true));
		}
		catch (Exception e) {}
		...
		for (int i = 0; i < args.length; i++) {
			...
			// if the unlimited arrows value is specified
			else if (arg.equals("-u") == true) {

				unlimitedArrows = Boolean.parseBoolean(args[i+1]);
				i++;
		...
		try {
			...
			for (int currTrial = 0; currTrial < numTrials; currTrial++) {
				...
				Simulation trial = new Simulation(wumpusEnvironment, maxSteps, nonDeterministicMode, unlimitedArrows); //, outputWriter, nonDeterministicMode);
